...
class UpdateJobRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'job_min_salary'      => ['required', 'integer', 'min:0'],
            'job_max_salary'      => ['required', 'integer', 'min:0', 'gte:job_min_salary'],

            'job_work_type'       => ['required', 'string'],
            'job_employment_type' => ['required', 'string'],
            'job_level'           => ['required', 'string'],
            'job_remote_option'   => ['required', 'string'],

            'job_location'        => ['required', 'integer', 'exists:cities,id'],
            'job_address'         => ['required', 'string', 'max:255'],

            'job_annual_leave'    => ['nullable', 'integer', 'min:0'],
            'job_open_positions'  => ['required', 'integer', 'min:1'],

            'is_active'           => ['required', 'boolean'],

            'job_languages'       => ['nullable', 'array'],
            'job_languages.*'     => ['integer', 'exists:languages,id'],
        ];
    }
    ...
    public function messages(): array
    {
        return [
            ...
            'is_active.required' => 'Моля, изберете статус на обявата.',
            'is_active.boolean'  => 'Статусът на обявата е невалиден.',
